...
# ── Gap-finder lookback gate (audit 2026-05-05, updated to 3-day default) ──
# Gap-finders (Tavily, Exa, OpenAI search, Gemini search, Claude search)
# routinely return older recalls because search engines surface whatever they
# indexed. This gate enforces a hard "no older than today - GAP_LOOKBACK_DAYS"
# cutoff for gap-finder rows.
#
# Audit 2026-05-05 (operator decision): tightened from 5 to 3 days. Search-
# engine results 4+ days old are mostly noise; recalls fresher than 3 days
# are still discoverable on the next gap-finder run if the regulator scraper
# missed them.
#
# Set via env var GAP_LOOKBACK_DAYS (default 3). A value of 0 disables.
_GAP_LOOKBACK_DAYS = int(os.getenv("GAP_LOOKBACK_DAYS", "3"))

# ── Scraper lookback gate (audit 2026-05-05, NEW) ─────────────────────────
# Regulator scrapers (RASFF, RappelConso, USDA, FDA, CFIA, BVL, FSANZ, etc.)
# already pull from official APIs with date filters, but operator policy
# says: nothing older than 7 days enters the pipeline regardless of source.
# This is a safety net — same hard-reject pattern as gap-finders, but
# with a more lenient window because regulators sometimes legitimately
# publish a recall 5-6 days late.
#
# Set via env var SCRAPER_LOOKBACK_DAYS (default 7). 0 disables.
_SCRAPER_LOOKBACK_DAYS = int(os.getenv("SCRAPER_LOOKBACK_DAYS", "7"))

# Source-name patterns that MIGHT mark a row as gap-finder origin. These
# are the fallback labels used when a URL doesn't match any whitelisted
# regulator domain (e.g. "Exa-daily" when daily_recall_search hits an
# unknown domain).
#
# In practice, gap-finders use HOST_TO_SOURCE lookup to label rows with
# the REAL regulator name (FDA, USDA FSIS, RASFF, etc.) — so the Source
# field alone CANNOT distinguish gap-finder rows from regulator-scraper
# rows. The reliable signal is the Notes stamp: every gap-finder writes
# "[via <Engine> gap-finder, ...]" or "[via <Engine> daily search, ...]"
# into Notes when it produces a row.
_GAP_FINDER_SOURCE_PATTERNS = (
    "tavily",        # explicit fallback labels
    "exa-daily",
    "exa daily",
    "openai-search",
    "gpt-4o",
...
# Notes-stamp patterns — THE reliable signal. Every gap-finder writes one
# of these into the Notes field when it produces a Recall row.
_GAP_FINDER_NOTES_PATTERNS = (
    "[via tavily gap-finder",
    "[via exa gap-finder",
    "[via exa daily search",
    "[via gemini gap-finder",
    "[via claude gap-finder",
    "[via openai gap-finder",
    "[via openai daily search",
    "[via perplexity",
    "gap-finder, deterministic extract",
    "+ google search]",          # gap_finder_gemini stamp
)


def _is_gap_finder_source(source: str, notes: str = "") -> bool:
    """Return True if the row is gap-finder origin.

    Checks BOTH the Source field (fallback labels) AND the Notes field
    (reliable stamps). Either signal is sufficient to mark the row as
    gap-finder, so the 3-day lookback applies.

    Notes signal is preferred because gap-finders normally relabel Source
    with the canonical regulator name (FDA, USDA FSIS, RASFF, etc.) via
    HOST_TO_SOURCE URL lookup. So a gap-finder-discovered FDA recall has
    Source="FDA" — indistinguishable from a real FDA-scraper row by Source
    alone. The "[via X gap-finder]" Notes stamp is what disambiguates.
    """
    s = (source or "").lower()
    n = (notes or "").lower()
    if any(p in s for p in _GAP_FINDER_SOURCE_PATTERNS):
        return True
    if any(p in n for p in _GAP_FINDER_NOTES_PATTERNS):
        return True
    return False
...
def validate_pending_row(
    ...
    # ── REJECT: gap-finder row dated older than GAP_LOOKBACK_DAYS ──────
    # Audit 2026-05-05 — gap-finders frequently return old recalls (search
    # engine indexes are stale).
    notes_str = str(row.get("Notes", "") or "")
    if (_GAP_LOOKBACK_DAYS > 0 and date_str
            and _is_gap_finder_source(source, notes_str)):
        from datetime import date as _date, timedelta as _td
        try:
            row_date = _date.fromisoformat(date_str[:10])
            cutoff = _date.today() - _td(days=_GAP_LOOKBACK_DAYS)
            if row_date < cutoff:
                return False, (
                    f"Pending gate: gap-finder row dated {date_str[:10]} is older "
                    f"than {_GAP_LOOKBACK_DAYS}-day window (cutoff {cutoff.isoformat()})"
                )
        except (ValueError, TypeError):
            # Malformed date — let it through; other checks will catch it
            pass

    # ── REJECT: regulator scraper row dated older than SCRAPER_LOOKBACK_DAYS ──
    # Audit 2026-05-05 — operator policy: nothing older than 7 days enters
    # the pipeline, regardless of source. This is the regulator-scraper
    # counterpart of the gap-finder gate above. Regulators DO sometimes
    # legitimately publish recalls 2-3 days late — that's why this window
    # is more lenient than the gap-finder window. Beyond 7 days the row
    # is too stale to be useful in our 7-day dashboard.
    if (_SCRAPER_LOOKBACK_DAYS > 0 and date_str
            and not _is_gap_finder_source(source, notes_str)):
        from datetime import date as _date, timedelta as _td
        try:
            row_date = _date.fromisoformat(date_str[:10])
            cutoff = _date.today() - _td(days=_SCRAPER_LOOKBACK_DAYS)
            if row_date < cutoff:
                return False, (
                    f"Pending gate: regulator-scraper row dated {date_str[:10]} "
                    f"is older than {_SCRAPER_LOOKBACK_DAYS}-day window "
