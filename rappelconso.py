    ...
    def scrape(self, since_days: int = 30) -> List[Recall]:
        ...
        for rec in r.json().get("results", []):
            try:
                ...
                _raw_company = rec.get("nom_de_la_societe_responsable_de_la_commercialisation", "") or ""
                _raw_brand   = rec.get("nom_de_la_marque_du_produit", "—") or "—"
                _co, _br = normalise_company_brand(_raw_company, _raw_brand)

                # Fallback: when the API returns empty Company (data-entry
                # gap — e.g. fiche 22176 Herta), use the Brand as Company.
                # On RappelConso, the brand is typically the legal entity
                # responsible for the recall (Herta, Carrefour France etc.),
                # not just a marketing label. Added 2026-05-04 per
                # docs/fsis_reviewer_prompts.md §2.
                _co_clean = (_co or "").strip()
                _br_clean = (_br or "").strip()
                _GARBAGE_COMPANY = {"", "—", "-", "n/a", "na",
                                    "non communique", "non communiqué",
                                    "inconnu", "unknown"}
                if _co_clean.lower() in _GARBAGE_COMPANY:
                    if _br_clean and _br_clean.lower() not in {"—", "-", "unbranded", "sans marque"}:
                        # Run the brand through normalisation as a Company
                        _co, _ = normalise_company_brand(_br_clean, _br_clean)
