    ...
    for n, idx in enumerate(work_idx, 1):
        ...
        if dec == "confirm":
            ...
            if _ref:
                ...
                note = str(row.get("Notes", "")).strip()
                tag = (f"[url-guard {today}: reviewer 1 tried to reject this "
                       f"row as {_why[:110]!r}; refused — the official URL is "
                       f"on the row. Left in Pending for a later run.]")
