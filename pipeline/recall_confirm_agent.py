...
def _norm_url(u: Any) -> str:
    ...
    for p in ("https://", "http://", "www."):
        if u.startswith(p):
            u = u[len(p):]
    # admin.ch newsroom (2026-09-07): one federal release is served at /de/,
    # /fr/ and /it/, and mirrored on every department host, under the same
    # message id. The German page of the OSAV Nautica trout warning was
    # published beside the French page already in Recalls. merge_master's
    # dedup key already collapses these; this makes the confirmer's own
    # duplicate test agree with it.
    m = _re.match(r"^[a-z0-9.-]*admin\.ch/(?:[a-z]{2}/)?newnsb/([a-z0-9]+)", u)
    if m:
        return "admin.ch/newnsb/" + m.group(1)
    return u
...
def main() -> int:
    ...
    for row in confirmed:
        ...
        if idx is None:
            continue
        # RECORD THE STATUS THE ROW ACTUALLY HELD (fix 2026-09-13).
        #
        # This stamp was hard-coded to "pending_gap_v3 → pending" for every
        # confirmed row, whatever the row's real prior status. Since the lane
        # above was widened on 2026-09-04 to admit ANY pending status, that
        # string is usually not what happened — and reviewer 2, the only
        # thing that sets pending_gap_v3, has banked nothing since at least
        # 2026-09-02 (replayed across every workbook commit since: the
        # pending_gap_v3 queue is 0 at every one). So every stamp written in
        # that window asserts a reviewer-2 hand-off that did not occur, on
        # rows reviewer 2 never saw. The register's provenance trail is read
        # by people deciding whether a row was checked; it has to say what
        # happened, not what used to happen.
        _prior = str(full_pending[idx].get("Status", "")).strip() or "(no status)"
        full_pending[idx]["Status"] = "pending"      # now promotable
        n = str(full_pending[idx].get("Notes", "")).strip()
        stamp = (f" [confirm-agent {today}: {_prior} → pending; "
    ...
    # SAY WHO ACTUALLY DID WHAT (fix 2026-09-13, same defect as the stamp
    # above). These two strings are what the operator reads in the archive
    # and in the Thursday email, and both asserted a reviewer-2 action:
    #
    #   "Confirmer: reviewer 2 approved but <problem>"
    #   "Rejected by reviewer 2"
    #
    # Neither is knowable here. The lane was widened on 2026-09-04 to admit
    # any pending status, so most rows reaching this point were never in
    # reviewer 2's hands — and reviewer 2 has banked nothing since at least
    # 2026-09-02 (its pending_gap_v3 queue is empty at every workbook commit
    # since). The first string is why 37 archived rows read as though two
    # stages agreed to drop them, several of which were genuine Tier-1
    # Listeria recalls with nothing wrong but an empty field.
    #
    # What IS known here is the row's status when this agent found it, so
    # that is what gets recorded. A row that really did come from reviewer 2
    # still says so, because its status says so.
    for row, probs in blocked:
        idx = by_url.get(str(row.get("URL", "")).strip())
        if idx is None:
            continue
        _from = str(full_pending[idx].get("Status", "")).strip() or "(no status)"
        rejected_flags[idx] = (f"Confirmer rejected (row was at {_from}): "
                               + "; ".join(probs[:2]))[:280]

    for row in already_rejected:
        idx = by_url.get(str(row.get("URL", "")).strip())
        if idx is not None and idx not in rejected_flags:
            rejected_flags[idx] = ("Arrived already marked rejected; "
    ...
    # AND THE OTHER HALF OF THE LEDGER (audit 2026-09-13).
    #
    # The block above mirrors EVICTIONS into Weekly_Rejected. Promotions were
    # never mirrored into Weekly_Review, and this agent is the only promoter
    # left: merge_master runs in janitor mode unless MERGE_MASTER_PROMOTE=1
    # (which the hourly merge-master.yml does not set, despite its commit
    # message reading "hourly Pending→Recalls merge"), and the two call sites
    # of record_promotions are claude_check.py and that disabled merge_master
    # branch. So nothing has written Weekly_Review since this agent took over
    # publishing.
    #
    # The cost, measured on the week of 2026-09-07..13: 62 rows entered
    # Recalls — 35 through this agent and 27 by operator review — and the
    # Sunday manual-review email reported "0 recalls added · No new recalls
    # promoted into the Recalls sheet this week", because Weekly_Review held
    # only its header. The freshness guard in the email was right and fired
    # ("JSON is 167.5h old"); the sheet behind it was simply never written.
    #
    # Same shape, same fix, same failure mode as the rejection mirror above:
    # save_xlsx_with_pending() does not do it for you, so the caller must.
    try:
        from pipeline.weekly_review_capture import record_promotions
        # json_path passed explicitly for the same reason the rejection
        # call above does it: the module-level default is resolved from the
        # package ROOT at import time, not from the workbook this run was
        # given, so it would write beside the wrong file if --xlsx moves.
        n_rev = record_promotions(new_approved, xlsx_path=xlsx,
                                  json_path=xlsx.parent / "weekly-review-latest.json")
        print(f"  (mirrored {n_rev} promotion(s) to Weekly_Review)")
    except Exception as e:                                   # noqa: BLE001
        print(f"  !! could not record promotions: {type(e).__name__}: {e}")
