...
def main() -> int:
    ...
    for row in confirmed:
        ...
        if idx is None:
            continue
        # THE STAMP MUST NAME THE STATUS THE ROW ACTUALLY HELD (2026-09-13).
        # This line used to interpolate the A2_APPROVED constant, so every
        # published row was stamped "pending_gap_v3 → pending" — asserting
        # that reviewer 2 had reviewed and banked it — whatever status the
        # row really carried. Reviewer 2 has not written a stamp to any
        # published row since 2026-08-10 (reviewer 1: 2026-08-05), and every
        # row this agent published in that window was at plain "pending"
        # straight from a scraper. So the audit trail claimed a review that
        ...
        stamp = (f" [confirm-agent {today}: {prior} → pending; "
                 ...
                 + ("" if prior == A2_APPROVED
                    else "; reviewer 2 did NOT review this row") + "]")
    ...
    for row, probs in blocked:
        ...
        rejected_flags[idx] = (f"Confirmer: {_who} "
                               + "; ".join(probs[:2]))[:280]

    for row in already_rejected:
        idx = by_url.get(str(row.get("URL", "")).strip())
        if idx is not None and idx not in rejected_flags:
            # The companion of the correction above, missed when that one
            # was made (fix 2026-09-14). This row arrived at Status
            # "rejected" — by whom is not recorded anywhere this agent can
            # see, and attributing it to reviewer 2 is a guess that reads in
            # the archive as a fact. Reviewer 2 has banked nothing since
            # 2026-09-02, so for every row archived in that window the guess
            # is also wrong.
