...
LISTING_URLS = (
    "https://www.ispch.gob.cl/alertas-alimentarias/",
    "https://www.achipia.gob.cl/",
    "https://www.ispch.gob.cl/",
)
...
def _try_fetch(url: str) -> str | None:
    try:
        ...
        print(f"  [WARN] ISP/ACHIPIA fetch failed: {url} — {e}")
        return None
...
def fetch(limit: int = 25) -> list[Record]:
    ...
    fetched = 0
    print(f"  enriching up to {min(len(links), _DETAIL_CAP)} ISP/ACHIPIA detail "
    ...
    for slug, list_title, url_full in links:
        ...
        rec = Record(
            source_id=f"ISP-{slug}",
            country_code="cl",
            country_name="Chile",
            authority="ISP/ACHIPIA",
...
CHILE = FeedSource(
    code="chile",
    name_en="Chile",
    authority_short="ISP/ACHIPIA",
    fetcher=fetch,
    region="Latin America",
    timezone="America/Santiago",
    run_local_hour=9,
    cron_utc_offsets=(13,),
    gnews_authority="Chile",
    gnews_terms=(
    ...
    gnews_hl="es-CL", gnews_gl="CL", gnews_ceid="CL:es",
    gnews_days_back=7,
