...
def _try_fetch(url: str) -> str | None:
    try:
        ...
        print(f"  [WARN] COFEPRIS fetch failed: {url} — {e}")
        return None
...
def _fetch_detail(url: str):
    try:
        ...
        print(f"  [WARN] COFEPRIS detail fetch failed: {url} — {e}")
        return "", None
...
def fetch(limit: int = 25) -> list[Record]:
    ...
    fetched = 0
    print(f"  enriching up to {min(len(links), _DETAIL_CAP)} COFEPRIS detail "
...
MEXICO = FeedSource(
    ...
    gnews_hl="es-MX", gnews_gl="MX", gnews_ceid="MX:es",
    gnews_days_back=7,
    ...
    authority_domain="gob.mx/cofepris",
    authority_url_pattern=r"(prensa|articulos|acciones-y-programas)/[^/]+",
)
