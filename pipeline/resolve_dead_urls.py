...
classified URL_FIX (HTTP 404/301/302), asks OpenAI gpt-4o-mini to propose
the CURRENT canonical recalls-page URL on each agency's domain, HTTP-
verifies each proposal, and writes a patch file (JSON).
...
Total cost: ~$0.05 for all 26 URL_FIX agencies (gpt-4o-mini, 1 call each).
"""
...
Your task: propose the CURRENT official listing page for food recalls / alerts / withdrawals on this agency's website.

Rules:
1. Must be on the SAME root domain as the broken URL (or a known sibling domain if the agency restructured — e.g. moved from www.foo.gov to alerts.foo.gov).
2. Must be a LISTING page (index of many recalls), NOT a single-recall page and NOT the agency's homepage.
3. If the agency has both an EN and a local-language version, prefer the EN version unless the previous URL was local-language.
4. If there's no obvious equivalent page (agency deprecated recall publishing, moved to a different platform like Twitter, etc.), return empty string.
5. Do NOT guess. If you can't be confident, return empty string with reasoning.
...
def propose_new_url(agency: str, country: str, bad_url: str) -> Optional[Dict]:
    ...
    txt = _call_openai(prompt, system=RESOLVE_SYSTEM, max_tokens=400)
    if not txt:
        return None
    txt = txt.strip()
    if txt.startswith("```"):
    ...
    if not new_url:
        return None  # model returned empty — no proposal
...
def main():
    ...
    for i, row in enumerate(targets, 1):
        ...
        if not proposal:
            log.info("    -> no proposal / OpenAI gave up")
            fixes.append({
                "agency": agency, "country": country,
                "old_url": bad_url, "new_url": None,
                "status": "NO_PROPOSAL",
                "detail": "OpenAI could not propose a URL",
            })
            continue
