...
def enforce_tier1(row: dict) -> dict:
    ...
    # MOISTURE GATE REMOVED (operator decision 2026-08-24): bare "Bacillus
    # cereus" is Tier 1 in EVERY matrix, wet or dry.
    #
    # The gate forced Tier 1 only in low-moisture products, on the theory that
    # cereulide forms in rice/pasta/powder and a chilled wet product is safe
    # until its use-by. The NVWA row broke that theory:
    #
    #     Milbona High Protein Pudding Chocolate Flavour, 200 g
    #     (THT 14-09-2026)          B. cereus, recalled 2026-08-19
    #
    # THT is "ten minste houdbaar tot" — BEST BEFORE. Under EU FIC rules a
    # microbiologically highly perishable food must carry a USE BY date
    # (Dutch TGT), so best-before means the product sits on an AMBIENT shelf,
    # not in a chiller. The gate had it at Tier 2 as a "chilled dessert" it
    # never was.
    #
    # Rather than keep patching the matrix list product by product, the gate
    # goes: B. cereus is the organism that produces cereulide, an emetic toxin
    # that is heat-stable and preformed, and the register cannot tell from a
    # recall notice whether an emetic strain was involved. Tier 1 is the
    # honest default.
