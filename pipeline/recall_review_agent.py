...
3. Decide scope:
   - reject if: not a recall page; pre-2026 date; pet/animal food; hazard
     ...
     dead URL with no findable official page; duplicate of an existing recall.
   - approve if: it is a real 2026+ food recall and every REQUIRED field
     (Date, Company OR Brand, Product, Pathogen, URL) is verified against the
     page.
...
def main() -> int:
    ...
    if not commit:
        ...
        print("  approvals → Recalls, rejects → Weekly_Rejected, "
              "needs_human stays in Pending.")
        return 0

    # Write-back through the pipeline's own helpers so dedup + formatting
    ...
    try:
        ...
        print(f"ERROR importing write helpers: {e}", file=sys.stderr)
        print("Adjust the import to match your merge_master API.",
              file=sys.stderr)
        return 1

    approved_rows = [m for m, _ in results["approve"]]
    # promote_approved expects rows tagged as approved; wiring mirrors
    # claude_check's usage. Left explicit for the operator to confirm.
    print(f"\nWould promote {len(approved_rows)} rows to Recalls, "
          f"reject {len(results['reject'])} to Weekly_Rejected.")
    print("Write-back wiring is intentionally explicit — confirm the "
          "promote_approved / mark_rejected_with_counter calls match your "
          "merge_master signatures before enabling in production.")
    return 0
