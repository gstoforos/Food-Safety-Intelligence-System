...
class USDAFSISScraper(BaseScraper):
    ...
    # Keywords matched against FSIS `field_summary`. FSIS scope is meat,
    # poultry, and egg products — so the new categories still apply:
    # rodenticide residues can appear in livestock feed, heavy metals
    # accumulate in offal/organ meats, and glass/metal fragments happen
    # in processing lines. Extended April 2026.
    PATHOGEN_KEYWORDS = (
        ...
        # --- Heavy metals (anchored "lead" to avoid false positives) ---
        "lead contamin", "elevated lead", "levels of lead", "lead in product",
        "cadmium", "arsenic", "mercury contamin", "heavy metal",
    ...
    def scrape(self, since_days: int = 30) -> List[Recall]:
        ...
        log.info("USDA FSIS: %d hazard recalls (biological+chemical+physical)", len(out))
        return out
