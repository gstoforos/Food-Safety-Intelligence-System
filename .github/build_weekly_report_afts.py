...
def generate_analysis_claude(stats, recalls):
    ...
    if bot:
        co = ", ".join(
            ...
            for r in bot[:3])
        pathogen_label = bot[0].get("Pathogen", "Clostridium botulinum")
        pa = ("\n4. PROCESS AUTHORITY NOTE paragraph. Start with EXACTLY: "
              "'This window contains {n} incident(s) implicating Clostridium or botulinum "
              "toxin, with {co} cited for {path}.' Then state that any shelf-stable low-"
              "acid canned food (LACF, 21 CFR 113), acidified food (21 CFR 114), aseptic/"
              "UHT, hot-filled, or reduced-oxygen-packaged product must be reviewed to "
              "confirm the scheduled thermal process has been established, filed, and "
              "validated under a qualified process authority under FDA 21 CFR 113/114 "
              "(Form 2541/2541e), EU Reg. 852/2004+2073/2005, CFIA SFCR, FSANZ Food "
              "Standards Code Ch.3, Japan FSA. Identify the most common compliance gap: "
              "unfiled/outdated scheduled process, deviation resolved without qualified "
              "review, container/seal-integrity lapse, or formulation change (pH, a_w, "
              "salt, preservative) not re-evaluated. End by noting a Tier-1 / Class-I "
              "classification on a low-acid product reliably triggers a 21 CFR 108 "
              "process-filing audit and FDA Form 483 citations, with analogous EU/CFIA/"
              "FSANZ/MHLW enforcement."
              ).format(n=len(bot), co=co, path=pathogen_label)
...
{}
Tone: professional, process-engineering voice, no emojis, no bullets, no colons at paragraph starts. 3-5 sentences each. Preserve the word 'AFTS' exactly where referenced.""".format(
    ...
    return _fallback(stats, bot, recalls)
...
def _fallback(stats, bot, recalls=None):
    tp, tc = stats["top_pathogen"]; t = stats["total"]
    pct = round(tc/max(t,1)*100)

    # P1 — stats (W16 wording preserved)
    ...
    # P2 — pathogen-specific process-engineering analysis
    p2 = _pathogen_narrative(tp, pct)

    # P3 — jurisdiction-aware regulatory summary
    ...
    # P4 — Process Authority Note (fires on Clostridium / low-acid / aseptic / ROP-MAP
    # / UHT-HTST / hot-fill triggers). Mirrors W16 specificity: names the implicated
    # company+country+pathogen, cites FDA 2541/2541e + 21 CFR 108 + Form 483 plus
    # parallel EU/CFIA/FSANZ/MHLW frameworks.
    if bot:
        co = ", ".join("{} ({})".format(r.get("Company",""), _country_display(r.get("Country","")))
                        for r in bot[:3])
        pathogen_label = bot[0].get("Pathogen", "Clostridium botulinum")
        p4 = ("This window contains {} incident(s) implicating Clostridium or botulinum "
              "toxin, with {} cited for {}. Any shelf-stable low-acid canned food (LACF, "
              "21 CFR 113), acidified food (21 CFR 114), aseptic/UHT, hot-filled, or "
              "reduced-oxygen-packaged product in the affected category must be reviewed "
              "to confirm that the scheduled thermal process has been established, filed, "
              "and validated under the guidance of a qualified process authority \u2014 required "
              "under FDA 21 CFR 113/114 (with Form 2541 / 2541e scheduled-process filing), "
              "EU Reg. 852/2004 + 2073/2005, CFIA SFCR, FSANZ Food Standards Code Ch. 3, "
              "and Japan\u2019s Food Sanitation Act. The most common compliance gap behind "
              "recalls of this profile is an unfiled or outdated scheduled process, a "
              "process deviation resolved without qualified process authority review, a "
              "container or seal-integrity lapse, or a formulation change (pH, a_w, salt, "
              "preservative) introduced without re-evaluation. A Tier-1 / Class-I "
              "classification on a low-acid product reliably triggers a 21 CFR 108 process-"
              "filing audit and FDA Form 483 citations on the subsequent inspection, with "
              "analogous enforcement steps under EU, CFIA, FSANZ, and MHLW frameworks."
              ).format(len(bot), co, pathogen_label)
        paras.append(p4)
